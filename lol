local espLibrary = {
  ...
  -- Add these optimization variables
  local lastUpdate = 0
  local activeObjects = 0
  local cachedProperties = {}
  
  -- Optimize the updateESP function
  local function updateESP()
    -- Early return if ESP is disabled
    if not espLibrary.options.enabled then
        -- Hide all ESP objects when disabled
        for _, objects in next, espLibrary.espCache do
        ...
        -- Hide all chams
        for _, highlight in next, espLibrary.chamsCache do
            highlight.Enabled = false
        end
        
        -- Hide all custom objects
        for _, cache in next, espLibrary.objectCache do
    ...
    if currentTick - lastUpdate < (1 / espLibrary.options.updateRate) then
        return
    end
    ...
    local maxDistance = espLibrary.options.cullingDistance

    -- Update ESP with optimizations
    for player, objects in next, espLibrary.espCache do
        -- Skip if we've hit the object limit
        if activeObjects >= espLibrary.options.maxObjects then
            break
        end
        ...
        -- Quick distance check before doing expensive calculations
        local distance = (cameraPos - torso.Position).Magnitude
        ...
        if not cachedProperties[player] or updateTick % 30 == 0 then
            cachedProperties[player] = {
                team = espLibrary.getTeam(player),
                tool = espLibrary.getTool(player)
            }
        end

        local team, teamColor = unpack(cachedProperties[player].team)
        local tool = cachedProperties[player].tool
    ...
    -- Batch chams updates
    if updateTick % 2 == 0 then  -- Update chams less frequently
  ...
  -- Add a new variable to track if ESP is running
  local isRunning = false

  -- Add a function to start/stop the ESP rendering
  local function toggleESP(enabled)
    ...
    elseif not enabled and isRunning then
        -- Stop ESP and hide everything
        isRunning = false
  ...
  -- Modify the options table to use a metatable for the enabled property
  local options = espLibrary.options
  espLibrary.options = setmetatable({}, {
    __index = options,
  ...
  function espLibrary:Load(renderValue)
    -- Pre-allocate pools
    for type, pool in next, drawingPool do
        for i = 1, 50 do
            table.insert(pool, drawingNew(type))
        end
    end
  ...
  -- Update Unload function
  function espLibrary:Unload()
    -- Make sure to unbind the render step
    if isRunning then
        toggleESP(false)
    end
