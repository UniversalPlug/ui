local espLibrary = {
  ...
  function espLibrary.getCharacter(player)
    -- Cache the character lookup
    local character = player.Character
    if not character then return nil, nil end
    
    -- Cache part lookups and use FindFirstChild instead of multiple separate lookups
  ...
  function espLibrary.visibleCheck(character, position)
    ...
    -- Reuse raycast params object instead of creating new one each time
    if not espLibrary.raycastParams then
        espLibrary.raycastParams = raycastParamsNew()
        espLibrary.raycastParams.IgnoreWater = true
    end
    
    -- Update filter list
    espLibrary.raycastParams.FilterDescendantsInstances = { 
        espLibrary.getCharacter(localPlayer),
        currentCamera,
        character 
    }
    espLibrary.raycastParams.FilterType = Enum.RaycastFilterType.Blacklist
    
    -- Cache direction vector
    local direction = position - origin
    
    return not raycast(workspace, origin, direction, espLibrary.raycastParams)
  end
  ...
  function espLibrary:Load(renderValue)
    ...
    local espCache = self.espCache
    local chamsCache = self.chamsCache
    local objectCache = self.objectCache
    ...
    runService:BindToRenderStep("esp_rendering", renderValue or (Enum.RenderPriority.Camera.Value + 1), function()
        ...
        for player, objects in next, espCache do
            local character, torso = self.getCharacter(player)
            
            if character and torso then
                -- Cache position and distance calculations
                ...
                -- Early exit if not on screen
                if not (onScreen and size and position) then
                    for _, object in next, objects do
                        object.Visible = false 
                    end
                    continue
                ...
                local show = self.options.enabled and onScreen;
                ...
                objects.arrow.Visible = (not show and self.options.outOfViewArrows) and self.options.outOfViewArrows;
                ...
                objects.arrowOutline.Visible = (not show and self.options.outOfViewArrowsOutline) and self.options.outOfViewArrowsOutline;
        ...
        for player, highlight in next, chamsCache do
            local character, torso = self.getCharacter(player);
        ...
        for object, cache in next, objectCache do
