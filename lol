local espLibrary = {
  ...
  local function create(type, properties)
    local drawing = type == "Square" or type == "Text" or type == "Triangle" or type == "Line" or type == "Circle";
  ...
  function espLibrary.getBoxData(position, size)
    local torsoPosition, onScreen, depth = currentCamera:WorldToViewportPoint(position);
    local size = vector2New(
        math.clamp(size.X, espLibrary.options.minScaleFactorX, espLibrary.options.maxScaleFactorX),
        math.clamp(size.Y, espLibrary.options.minScaleFactorY, espLibrary.options.maxScaleFactorY)
    );
  ...
  function espLibrary.visibleCheck(character, position)
    ...
    return not workspace:Raycast(currentCamera.CFrame.Position, position - currentCamera.CFrame.Position, params);
  end
