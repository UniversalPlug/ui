local espLibrary = {
  ...
  -- Optimize object creation with a template system
  local templates = {
      triangle = {
          Thickness = 1
      },
  ...
  local function createFromTemplate(type, template, extraProps)
      local obj = create(type, template)
      if extraProps then
          for k,v in next, extraProps do
              obj[k] = v
          end
      end
      return obj
  ...
  function espLibrary.addEsp(player)
    ...
    local objects = {
        arrow = createFromTemplate("Triangle", templates.triangle),
        arrowOutline = createFromTemplate("Triangle", templates.triangle),
        bottom = createFromTemplate("Text", templates.text),
        tool = createFromTemplate("Text", templates.text),
        top = createFromTemplate("Text", templates.text),
        side = createFromTemplate("Text", templates.text, {Center = false}),
        boxFill = createFromTemplate("Square", templates.square, {Filled = true}),
        boxOutline = createFromTemplate("Square", templates.square, {Thickness = 3}),
        box = createFromTemplate("Square", templates.square),
        healthBarOutline = createFromTemplate("Square", templates.square, {Filled = true}),
        healthBar = createFromTemplate("Square", templates.square, {Filled = true}),
        lineoutline = createFromTemplate("Line", templates.square, {Thickness = 3}),
        line = createFromTemplate("Line", templates.square, {Thickness = 1})
    };
