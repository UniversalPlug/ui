local espLibrary = {
  ...
  -- Add these cached variables at the top with other variables
  ...
  function espLibrary.getBoundingBox(character, torso)
    if (not espLibrary.options.boundingBox) then
    ...
    -- Cache options
    local excludedNames = espLibrary.options.excludedPartNames
    local isDescending = espLibrary.options.boundingBoxDescending
    
    -- Use direct part iteration instead of GetDescendants/GetChildren
    local parts = isDescending and character:GetDescendants() or character:GetChildren()
    
    for i = 1, #parts do
        local part = parts[i]
        if part:IsA("BasePart") and not find(excludedNames, part.Name) then
  ...
  function espLibrary:Load(renderValue)
    ...
    runService:BindToRenderStep("esp_rendering", renderValue or (Enum.RenderPriority.Camera.Value + 1), function()
        ...
        for player, objects in next, self.espCache do
            ...
            -- Cache position and distance check early
            local torsoPos = torso.Position
            local distance = (currentCamera.CFrame.Position - torsoPos).Magnitude
            ...
            local onScreen, size, position, torsoPosition = self.getBoxData(torsoPos, Vector3.new(5, 6.5));
            ...
            local show = onScreen and options.enabled;
            ...
            objects.arrow.Visible = (not onScreen and options.outOfViewArrows) and show;
            objects.arrow.Filled = options.outOfViewArrowsFilled;
            objects.arrow.Transparency = options.outOfViewArrowsTransparency;
            ...
            objects.arrowOutline.Visible = (not onScreen and options.outOfViewArrowsOutline) and show;
            objects.arrowOutline.Filled = options.outOfViewArrowsOutlineFilled;
            objects.arrowOutline.Transparency = options.outOfViewArrowsOutlineTransparency;
            ...
            objects.top.Visible = show and options.names;
            objects.top.Font = options.font;
            objects.top.Size = options.fontSize;
            objects.top.Transparency = options.nameTransparency;
            objects.top.Color = color or options.nameColor;
            objects.top.Text = player.Name;
            objects.top.Position = round(position + vector2New(size.X * 0.5, -(objects.top.TextBounds.Y + 2)));
  
            objects.side.Visible = show and options.healthText;
            objects.side.Font = options.font;
            objects.side.Size = options.fontSize;
            objects.side.Transparency = options.healthTextTransparency;
            objects.side.Color = color or options.healthTextColor;
            objects.side.Text = health .. options.healthTextSuffix;
            objects.side.Position = round(position + vector2New(size.X + 3, -3));

            objects.tool.Visible = show and options.tool and tool ~= "None";
            objects.tool.Font = options.font;
            objects.tool.Size = options.fontSize;
            objects.tool.Transparency = options.toolTransparency;
            objects.tool.Color = color or options.toolColor;
            ...
            objects.bottom.Visible = show and options.distance;
            objects.bottom.Font = options.font;
            objects.bottom.Size = options.fontSize;
            objects.bottom.Transparency = options.distanceTransparency;
            objects.bottom.Color = color or options.distanceColor;
            objects.bottom.Text = tostring(round(distance)) .. options.distanceSuffix;
            objects.bottom.Position = round(position + vector2New(size.X * 0.5, size.Y + Distance_Offset));
  
            objects.box.Visible = show and options.boxes;
            objects.box.Color = color or options.boxesColor;
            objects.box.Transparency = options.boxesTransparency;
            objects.box.Size = size;
            objects.box.Position = position;
  
            objects.boxOutline.Visible = show and options.boxes;
            objects.boxOutline.Transparency = options.boxesTransparency;
            objects.boxOutline.Size = size;
            objects.boxOutline.Position = position;
  
            objects.boxFill.Visible = show and options.boxFill;
            objects.boxFill.Color = color or options.boxFillColor;
            objects.boxFill.Transparency = options.boxFillTransparency;
            objects.boxFill.Size = size;
            objects.boxFill.Position = position;
  
            objects.healthBar.Visible = show and options.healthBars;
            ...
            objects.healthBarOutline.Visible = show and options.healthBars;
            ...
            objects.line.Visible = show and options.tracers;
            ...
            objects.lineoutline.Visible = show and options.tracers;
        ...
        for player, highlight in next, self.chamsCache do
            ...
            if (character and torso) then
                ...
                local canShow = options.enabled and options.chams;
                local team, teamColor = self.getTeam(player);
                local color = self.options.teamColor and teamColor or nil;
