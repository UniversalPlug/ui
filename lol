local espLibrary = {
  ...
  -- Add these cached values at the top after the existing variables
  ...
  function espLibrary.addEsp(player)
    ...
    local objects = {
        arrow = optimizedCreate("Triangle", {
            Thickness = 1,
        }),
        arrowOutline = optimizedCreate("Triangle", {
            Thickness = 1,
        }),
        bottom = optimizedCreate("Text", {
            Center = true,
            Size = 13,
            Outline = true,
            OutlineColor = color3New(),
        ...
        side = optimizedCreate("Text", {
            Size = 13,
            Outline = true,
            OutlineColor = color3New(),
            Font = 2,
        }),
        boxFill = optimizedCreate("Square", {
            Thickness = 1,
            Filled = true,
        }),
        boxOutline = optimizedCreate("Square", {
            Thickness = 3,
            Color = color3New()
        }),
        box = optimizedCreate("Square", {
            Thickness = 1
        }),
        healthBarOutline = optimizedCreate("Square", {
            Thickness = 1,
            Color = color3New(),
            Filled = true
        }),
        healthBar = optimizedCreate("Square", {
            Thickness = 1,
            Filled = true
        }),
        lineoutline = optimizedCreate("Line", {Thickness = 3}),
        line = optimizedCreate("Line", {Thickness = 1})
    };
  ...
  function espLibrary.addChams(player)
    ...
    espLibrary.chamsCache[player] = optimizedCreate("Highlight", {
        Parent = screenGui,
    });
  end
  ...
  local function updateESP()
    ...
    -- Update ESP with batched property updates
    for player, objects in next, espLibrary.espCache do
        ...
        -- Update objects with batched property sets
        if show then
            -- Update text objects
            local textProps = {
                Visible = true,
