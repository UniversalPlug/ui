local espLibrary = {
  ...
  -- Cache frequently used functions and values at the top
  local find = table.find
  local round = math.floor
  local worldToViewportPoint = currentCamera.WorldToViewportPoint
  local getCharacter = players.GetCharacter
  ...
  -- Optimize getCharacter function
  function espLibrary.getCharacter(player)
    local character = player.Character
    if not character then return nil, nil end
    local torso = getRootPart(character)
  ...
  -- Optimize visibility check with cached params
  ...
  function espLibrary.visibleCheck(character, position)
    ...
    return not raycast(workspace, origin, position - origin, rayParams)
  end
  
  -- Optimize getBoundingBox with cached values
  function espLibrary.getBoundingBox(character, torso)
    if not espLibrary.options.boundingBox then
        return torso.Position, vector2New(
            espLibrary.options.scaleFactorX,
            espLibrary.options.scaleFactorY
        )
    end

    local minVec = vector3New(inf, inf, inf)
    local maxVec = vector3New(-inf, -inf, -inf)
    
    for _, part in next, espLibrary.options.boundingBoxDescending and 
        getDescendants(character) or getChildren(character) do
        
        if isA(part, "BasePart") and 
           not find(espLibrary.options.excludedPartNames, part.Name) then
            
            local cf = part.CFrame
            local size = part.Size
            local x, y, z, r00, r01, r02, r10, r11, r12, r20, r21, r22 = getComponents(cf);
  
            local wiseX = 0.5 * (abs(r00) * size.X + abs(r01) * size.Y + abs(r02) * size.Z);
            local wiseY = 0.5 * (abs(r10) * size.X + abs(r11) * size.Y + abs(r12) * size.Z);
            local wiseZ = 0.5 * (abs(r20) * size.X + abs(r21) * size.Y + abs(r22) * size.Z);
  
            minVec.X = minVec.X > x - wiseX and x - wiseX or minVec.X;
            minVec.Y = minVec.Y > y - wiseY and y - wiseY or minVec.Y;
            minVec.Z = minVec.Z > z - wiseZ and z - wiseZ or minVec.Z;
  
            maxVec.X = maxVec.X < x + wiseX and x + wiseX or maxVec.X;
            maxVec.Y = maxVec.Y < y + wiseY and y + wiseY or maxVec.Y;
            maxVec.Z = maxVec.Z < z + wiseZ and z + wiseZ or maxVec.Z;
        end
    end
    
    local oMin, oMax = vector3New(minVec.X, minVec.Y, minVec.Z),
                       vector3New(maxVec.X, maxVec.Y, maxVec.Z)
  ...
  function espLibrary:Load(renderValue)
    ...
    runService:BindToRenderStep("esp_rendering", renderValue or 
        ...
        for player, objects in next, self.espCache do
            ...
            local team = self.options.teamCheck and self.getTeam(player)
            if team and team == self.getTeam(localPlayer) then
                continue
            end

            local onScreen, size, position, torsoPosition = self.getBoxData(torso.Position, Vector3.new(5, 6.5));
            local canShow = onScreen and self.options.enabled;
            ...
            objects.arrow.Visible = (not canShow) and self.options.outOfViewArrows;
            ...
            objects.arrowOutline.Visible = (not canShow) and self.options.outOfViewArrowsOutline;
