local espLibrary = {
  ...
  function espLibrary.createSkeletonLines()
    local lines = {}
    for i=1,15 do
        lines[i] = create("Line", {
            Thickness = 1,
  ...
  function espLibrary.addEsp(player)
    ...
    local objects = {
        ...
        skeletonLines = espLibrary.createSkeletonLines()
    };
  ...
  function espLibrary.removeEsp(player)
    ...
    if (espCache) then
        ...
        for index, object in next, espCache do
            -- Special handling for skeleton lines
            if index == "skeletonLines" then
    ...
    runService:BindToRenderStep("esp_rendering", renderValue or (Enum.RenderPriority.Camera.Value + 1), function()
        for player, objects in next, self.espCache do
            ...
            if (character and torso) then
                ...
                -- Add skeleton update
                self.updateSkeleton(character, objects, show, color)
  ...
  function espLibrary.updateSkeleton(character, objects, show, color)
    local lines = objects.skeletonLines
    ...
    if not (show and espLibrary.options.skeleton) then return end
    
    local lineCount = 1
    local function addLine(p1, p2)
        local line = lines[lineCount]
        local p1, vis1 = worldToViewportPoint(p1)
        local p2, vis2 = worldToViewportPoint(p2)
        ...
        if vis1 and vis2 then
            ...
            line.From = p1
            line.To = p2
            line.Color = color or espLibrary.options.skeletonColor  
            line.Transparency = espLibrary.options.skeletonTransparency
            
            line.outline.From = p1
            line.outline.To = p2
            line.outline.Transparency = espLibrary.options.skeletonTransparency
        end
    ...
    if not (head and humanoidRootPart and humanoid) then return end
    ...
    if humanoid.RigType == Enum.HumanoidRigType.R6 then
        ...
        if torso then
            if head then addLine(torso.Position, head.Position) end
            if leftArm then addLine(torso.Position, leftArm.Position) end
            if rightArm then addLine(torso.Position, rightArm.Position) end
            if leftLeg then addLine(torso.Position, leftLeg.Position) end
            if rightLeg then addLine(torso.Position, rightLeg.Position) end
        end
        ...
        if upperTorso then
            if head then addLine(upperTorso.Position, head.Position) end
            if lowerTorso then addLine(upperTorso.Position, lowerTorso.Position) end
            if leftUpperArm then addLine(upperTorso.Position, leftUpperArm.Position) end
            if rightUpperArm then addLine(upperTorso.Position, rightUpperArm.Position) end
        end
        
        if leftUpperArm and leftLowerArm then addLine(leftUpperArm.Position, leftLowerArm.Position) end
        if leftLowerArm and leftHand then addLine(leftLowerArm.Position, leftHand.Position) end
        if rightUpperArm and rightLowerArm then addLine(rightUpperArm.Position, rightLowerArm.Position) end
        if rightLowerArm and rightHand then addLine(rightLowerArm.Position, rightHand.Position) end
        
        if lowerTorso then
            if leftUpperLeg then addLine(lowerTorso.Position, leftUpperLeg.Position) end
            if rightUpperLeg then addLine(lowerTorso.Position, rightUpperLeg.Position) end
        end
        
        if leftUpperLeg and leftLowerLeg then addLine(leftUpperLeg.Position, leftLowerLeg.Position) end
        if leftLowerLeg and leftFoot then addLine(leftLowerLeg.Position, leftFoot.Position) end
        if rightUpperLeg and rightLowerLeg then addLine(rightUpperLeg.Position, rightLowerLeg.Position) end
        if rightLowerLeg and rightFoot then addLine(rightLowerLeg.Position, rightFoot.Position) end
    end
end
