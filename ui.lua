	...
	MakeDraggable = function(Dragger, Object, OnChange, OnEnd)
		...
		Dragger.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				Position = UserInputService:GetMouseLocation() -- Use GetTouchLocation for touch
		...
		UserInputService.InputChanged:Connect(function(Input)
			if StartPosition and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
				local Mouse = UserInputService:GetMouseLocation() -- Use GetTouchLocation for touch
...
Bracket.Assets = {
	...
	Colorpicker = function()
		...
		local function onTouchMoved(touch)
			...
			local colorPickerPosition = Colorpicker.AbsolutePosition
			local colorPickerSize = Colorpicker.AbsoluteSize

			-- Calculate the new position of the color selector
			local newX = math.clamp(position.X - colorPickerPosition.X, 0, colorPickerSize.X)
			local newY = math.clamp(position.Y - colorPickerPosition.Y, 0, colorPickerSize.Y)

			-- Update the color selector position
			Color.Position = UDim2.new(0, newX, 0, newY)
		end
		...
		Colorpicker.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.Touch then
				...
				Colorpicker.InputChanged:Connect(onTouchMoved)
			end
		end)
	...
	ColorpickerPalette = function()
		...
		-- Mobile touch support for Saturation/Value Picker
		local function onSVPickerTouchMoved(touch)
			local position = touch.Position
			local svPickerPosition = SVPicker.AbsolutePosition
			local svPickerSize = SVPicker.AbsoluteSize

			-- Calculate the new position of the pin
			local newX = math.clamp(position.X - svPickerPosition.X, 0, svPickerSize.X)
			local newY = math.clamp(position.Y - svPickerPosition.Y, 0, svPickerSize.Y)

			-- Update the pin position
			Pin.Position = UDim2.new(0, newX, 0, newY)

			-- Calculate the color based on the new position
			local hue = Hue.Position.Y.Scale
			local saturation = newX / svPickerSize.X
			local value = 1 - (newY / svPickerSize.Y)

			-- Update the color based on saturation and value
		...
		SVPicker.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.Touch then
				input.Changed:Connect(function()
					if input.UserInputState == Enum.UserInputState.End then
						SVPicker.InputChanged:Disconnect()
					end
				end)
				SVPicker.InputChanged:Connect(onSVPickerTouchMoved)
			end
		end)

		-- Mobile touch support for Hue Slider
		local function onHueTouchMoved(touch)
			local position = touch.Position
			local huePosition = Hue.AbsolutePosition
			local hueSize = Hue.AbsoluteSize

			-- Calculate the new position of the pin
			local newY = math.clamp(position.Y - huePosition.Y, 0, hueSize.Y)

			-- Update the pin position
			Pin.Position = UDim2.new(0, 0, 0, newY)

			-- Calculate the hue based on the new position
			local hue = newY / hueSize.Y

			-- Update the color based on hue
			local saturation = 1 - (Pin.Position.Y.Offset / svPickerSize.Y)
			local value = 1
			local color = Color3.fromHSV(hue, saturation, value)
			Color.BackgroundColor3 = color
		end
		...
		Hue.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.Touch then
				input.Changed:Connect(function()
					if input.UserInputState == Enum.UserInputState.End then
						Hue.InputChanged:Disconnect()
					end
				end)
				Hue.InputChanged:Connect(onHueTouchMoved)
			end
		end)
